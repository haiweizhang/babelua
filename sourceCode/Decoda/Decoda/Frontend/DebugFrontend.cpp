...
{
	HINSTANCE hInstance = GetCurrentInstanceHandle();
    if (!GetModuleFileName(hInstance/*NULL*/, path, maxPathLength))
    {
		return false;
    }
...
bool DebugFrontend::ProcessInitialization(const char* symbolsDirectory)
{
	...
	//add code
	if(g_CallbackEventInitialize != NULL)
	{
		g_CallbackEventInitialize(g_dwThreadId);
	}
	//end
