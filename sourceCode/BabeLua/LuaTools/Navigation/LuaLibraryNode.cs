...
namespace Microsoft.LuaTools.Navigation {
    internal class LuaLibraryNode : CommonLibraryNode {
        
        public LuaLibraryNode(LibraryNode parent, IScopeNode scope, string namePrefix, IVsHierarchy hierarchy, uint itemId)
            : base(parent, scope, namePrefix, hierarchy, itemId) { }

        protected LuaLibraryNode(LuaLibraryNode node) : base(node) { }

        protected LuaLibraryNode(LuaLibraryNode node, string newFullName) : base(node, newFullName) { }

        public override LibraryNode Clone() {
            return new LuaLibraryNode(this);
        }

        public override LibraryNode Clone(string newFullName) {
            return new LuaLibraryNode(this, newFullName);
        }
        ...
        private void GetFunctionDescription(FunctionDefinition def, Action<string, VSOBDESCRIPTIONSECTION, IVsNavInfo> addDescription) {
            ...
            for (int i = 0; i < def.Parameters.Count; i++) {
                ...
                if (curParam.DefaultValue != null) {
                    ...
                    // probably have a LuaAst walker for expressions or we should add ToCodeString()
                    // onto Lua ASTs so they can round trip
