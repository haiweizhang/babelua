...
namespace Microsoft.LuaTools.Language {
    /// <summary>
    ...
    public sealed class TextViewFilter : IOleCommandTarget, IVsTextViewFilter {
        ...
        public int GetDataTipText(TextSpan[] pSpan, out string pbstrText) {
            if (!_wpfTextView.TextBuffer.ContentType.IsOfType(LuaCoreConstants.ContentType)) {
                pbstrText = null;
            ...
            if (_wpfTextView.Selection.IsEmpty)
            {
                try
                {
                    var point = LineAndColumnNumberToSnapshotPoint(snapshot, pSpan[0].iStartLine, pSpan[0].iStartIndex);
                    var span = _wpfTextView.GetToken(point);
                    var start = span.Start;
                    var end = span.End;

                    SnapshotPointToLineAndColumnNumber(start, out pSpan[0].iStartLine, out pSpan[0].iStartIndex);
                    SnapshotPointToLineAndColumnNumber(end, out pSpan[0].iEndLine, out pSpan[0].iEndIndex);
                }
                catch(Exception e)
                {
