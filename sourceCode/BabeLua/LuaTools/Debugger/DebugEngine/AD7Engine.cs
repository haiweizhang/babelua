...
/*
using Microsoft.LuaTools.Debugger.Remote;
...
namespace Microsoft.LuaTools.Debugger.DebugEngine {
    ...
    [ComVisible(true)]
    [Guid("2977A1F2-E196-48F6-9974-9956DA56E0C4")]//[Guid("8355452D-6D2F-41b0-89B8-BB2AA2529E94")]
    public sealed class AD7Engine : IDebugEngine2, IDebugEngineLaunch2, IDebugProgram3, IDebugSymbolSettings100 {
        ...
        // The core of the engine is implemented by LuaProcess - we wrap and expose that to VS.
        private LuaProcess _process;
        
        // mapping between LuaProcess threads and AD7Threads
...
//        private string _webBrowserUrl;
        ...
        // Lua thread IDs can be 64-bit (e.g. when remotely debugging a 64-bit Linux system), but VS debugger APIs only work
        ...
        public const string DebugEngineId = "{D97F3CA6-F736-486C-988C-E634119EE5E7}";//"{EC1375B7-E2CE-43E8-BF75-DC638DE1F1F9}";
        ...
        /// <summary>
        /// Specifies options which should be passed to the Lua interpreter before the script.  If
        ...
        public AD7Engine() {            
            ...
            Debug.WriteLine("Lua Engine Created " + GetHashCode());
            _engines.Add(new WeakReference(this));
        }

        ~AD7Engine() {
            Debug.WriteLine("Lua Engine Finalized " + GetHashCode());
        ...
        private static bool IsDebuggingLuaOnly(IDebugProgram2 program) {
#if DEV11_OR_LATER
        ...
        int IDebugEngine2.Attach(IDebugProgram2[] rgpPrograms, IDebugProgramNode2[] rgpProgramNodes, uint celtPrograms, IDebugEventCallback2 ad7Callback, enum_ATTACH_REASON dwReason) {
            Debug.WriteLine("LuaEngine Attach Begin " + GetHashCode());

            AssertMainThread();
            Debug.Assert(_ad7ProgramId == Guid.Empty);

            if (celtPrograms != 1) {
                Debug.Fail("Lua debugging only supports one program in a process");
            ...
            if (_process == null) {                
                ...
                // Check whether we're debugging Lua alongside something else. If so, let Concord handle everything.
                if (!IsDebuggingLuaOnly(program)) {
                    _attached = true;
                    _mixedMode = true;
                    AD7EngineCreateEvent.Send(this);
                    AD7ProgramCreateEvent.Send(this);
                    Debug.WriteLine("LuaEngine Attach bailing out early - mixed-mode debugging");
                    return VSConstants.S_OK;
                }
/*
                // Check if we're attaching remotely using the Lua remote debugging transport
                var remoteProgram = program as LuaRemoteDebugProgram;
                ConnErrorMessages attachRes;
                if (remoteProgram != null) {
                    var remotePort = remoteProgram.DebugProcess.DebugPort;
                    attachRes = LuaRemoteProcess.TryAttach(remotePort.HostName, remotePort.PortNumber, remotePort.Secret, remotePort.UseSsl, SslErrorHandling.PromptUser, out _process);
                } else {
                    attachRes = LuaProcess.TryAttach(processId, out _process);
                }
...
//            info.bstrExe = _webBrowserUrl;
        ...
        int IDebugEngine2.DestroyProgram(IDebugProgram2 pProgram) {
            ...
            Debug.WriteLine("LuaEngine DestroyProgram");
        ...
        int IDebugEngine2.RemoveSetException(EXCEPTION_INFO[] pException) {
            ...
            for (int i = 0; i < pException.Length; i++) {
                if (pException[i].guidType == DebugEngineGuid) {
                    sendUpdate = true;
                    if (pException[i].bstrExceptionName == "Lua Exceptions") {
                        _defaultBreakOnExceptionMode = (int)enum_EXCEPTION_STATE.EXCEPTION_STOP_USER_UNCAUGHT;
                    } else {
        ...
        int IDebugEngineLaunch2.CanTerminateProcess(IDebugProcess2 process) {
            ...
            Debug.WriteLine("LuaEngine CanTerminateProcess");
        ...
        private static int _launchNum = 0;
        public static void UpdateUserDataDebug()
        {
            _launchNum = _launchNum + 1;
            if (_launchNum == 3)
            {
                Logger.UpdateUserData("debug");
            }
        }
        ...
        int IDebugEngineLaunch2.LaunchSuspended(string pszServer, IDebugPort2 port, string exe, string args, string dir, string env, string options, enum_LAUNCH_FLAGS launchFlags, uint hStdInput, uint hStdOutput, uint hStdError, IDebugEventCallback2 ad7Callback, out IDebugProcess2 process) {
            ...
            Debug.WriteLine("--------------------------------------------------------------------------------");
            Debug.WriteLine("LuaEngine LaunchSuspended Begin " + launchFlags + " " + GetHashCode());
            AssertMainThread();
            Debug.Assert(_events == null);
            Debug.Assert(_process == null);
            Debug.Assert(_ad7ProgramId == Guid.Empty);
...
//            LuaLanguageVersion version = DefaultVersion;
//            LuaDebugOptions debugOptions = LuaDebugOptions.None;
...
/*
                        ...
                        switch (setting[0]) {
                            ...
                            case WaitOnAbnormalExitSetting:
                                ...
                                if (Boolean.TryParse(setting[1], out value) && value) {
                                    debugOptions |= LuaDebugOptions.WaitOnAbnormalExit;
                                }
                                break;
                            case WaitOnNormalExitSetting:
                                if (Boolean.TryParse(setting[1], out value) && value) {
                                    debugOptions |= LuaDebugOptions.WaitOnNormalExit;
                                }
                                break;
                            case RedirectOutputSetting:
                                if (Boolean.TryParse(setting[1], out value) && value) {
                                    debugOptions |= LuaDebugOptions.RedirectOutput;
                                }
                                break;
                            case BreakSystemExitZero:
                                if (Boolean.TryParse(setting[1], out value) && value) {
                                    debugOptions |= LuaDebugOptions.BreakOnSystemExitZero;
                                }
                                break;
                            case DebugStdLib:
                                if (Boolean.TryParse(setting[1], out value) && value) {
                                    debugOptions |= LuaDebugOptions.DebugStdLib;
                                }
                                break;
                            ...
                            case EnableDjangoDebugging:
                                if (Boolean.TryParse(setting[1], out value) && value) {
                                    debugOptions |= LuaDebugOptions.DjangoDebugging;
                                }
                                break;
                        }
                    }
            ...
            if (attachRunning && Guid.TryParse(exe, out processId)) {
                ...
                _process = new LuaProcess(version, exe, args, dir, env, interpreterOptions, debugOptions, dirMapping);
            }
...
*/
            ...
            _process.InitProcess((int)processId);
//            _process = new LuaProcess(exe, args, dir, env, interpreterOptions, debugOptions, dirMapping);
...
//            AttachEvents(_process);
            ...
            Debug.WriteLine("LuaEngine LaunchSuspended returning S_OK");
...
/*
        ...
        private const LuaLanguageVersion DefaultVersion = LuaLanguageVersion.V27;

        private static LuaLanguageVersion GetLanguageVersion(string options) {
            LuaLanguageVersion langVersion;
            if (options == null || !Enum.TryParse<LuaLanguageVersion>(options, out langVersion)) {
...
*/
        ...
        int IDebugEngineLaunch2.ResumeProcess(IDebugProcess2 process) {
            ...
            Debug.WriteLine("Lua Debugger ResumeProcess Begin");
        ...
        int IDebugEngineLaunch2.TerminateProcess(IDebugProcess2 process) {
            ...
            Debug.WriteLine("LuaEngine TerminateProcess");
        ...
        public int CauseBreak() {
            ...
            Debug.WriteLine("LuaEngine CauseBreak");
            AssertMainThread();
        ...
        public int Continue(IDebugThread2 pThread) {
            ...
            Debug.WriteLine("LuaEngine Continue " + thread.GetDebuggedThread().Id);
            AssertMainThread();
        ...
        public int Detach() {
            Debug.WriteLine("LuaEngine Detach");
            AssertMainThread();
        ...
        public int Terminate() {
            Debug.WriteLine("LuaEngine Terminate");
        ...
        private void AttachEvents(LuaProcess process) {
